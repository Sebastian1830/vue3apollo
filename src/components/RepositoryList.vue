<template>
  ...
  <div v-else-if="repositories.length">
    <ul class="md:block max-w-6xl mx-auto">
      <li class="relative mb-8" v-for="repository in repositories" :key="repository.node.id">
...
export default defineComponent({
  ...
  setup(props: { searchOptions: Record<string, any> }) {
    ...
    const { result, loading, error } = useQuery<{search: SearchResultItemConnection}>(SEARCH_REPOS, searchOptions);
    const repositories = useResult(result, [], data => data.search && data.search.edges);
